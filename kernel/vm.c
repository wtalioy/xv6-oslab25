...
void global_kvminit() {
  ...
  // uart registers
  global_kvmmap(UART0, UART0, PGSIZE, PTE_R | PTE_W);

  // virtio mmio disk interface
  global_kvmmap(VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);

  // CLINT
  global_kvmmap(CLINT, CLINT, 0x10000, PTE_R | PTE_W);

  // PLIC
  global_kvmmap(PLIC, PLIC, 0x400000, PTE_R | PTE_W);

  // map kernel text executable and read-only.
  global_kvmmap(KERNBASE, KERNBASE, (uint64)etext - KERNBASE, PTE_R | PTE_X);

  // map kernel data and the physical RAM we'll make use of.
  global_kvmmap((uint64)etext, (uint64)etext, PHYSTOP - (uint64)etext, PTE_R | PTE_W);

  // map the trampoline for trap entry/exit to
  // the highest virtual address in the kernel.
  global_kvmmap(TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
}
...
void global_kvmmap(uint64 va, uint64 pa, uint64 sz, int perm) {
  if (mappages(kernel_pagetable, va, sz, pa, perm) != 0) panic("global_kvmmap");
}
...
void _dfs_vmprint(pagetable_t pagetable, int level, uint64 va_prefix) {
  char prefix[12] = "";
  for (int i = 0; i < 2 - level; i++) safestrcpy(prefix + strlen(prefix), "||   ", 6);
  strncpy(prefix + strlen(prefix), "||", 3);
  ...
  for (uint64 i = 0; i < 512; i++) {
    ...
    uint64 child;
    va = va_prefix | (i << PXSHIFT(level));
    if (level != 0) {
      child = PTE2PA(pte);
      printf("%sidx: %d: pa: %p, flags: ", prefix, i, child);
    } else {
      printf("%sidx: %d: va: %p -> pa: %p, flags: ", prefix, i, va, PTE2PA(pte));
    }
    printf("%s%s%s%s\n", pte & PTE_R ? "r" : "-", pte & PTE_W ? "w" : "-", pte & PTE_X ? "x" : "-",
           pte & PTE_U ? "u" : "-");
    if (level != 0) _dfs_vmprint((pagetable_t)child, level - 1, va);
  }
}

void vmprint(pagetable_t pagetable) {
  printf("page table %p\n", pagetable);
  _dfs_vmprint(pagetable, 2, 0);
}
...
// Synchronize kernel page table with user page table
int copyuvm2kvm(pagetable_t upgtbl, pagetable_t kpgtbl, uint64 oldsz, uint64 newsz) {
  pte_t *upte;
  uint64 va, pa;
  uint flags;
  ...
  // If shrinking, unmap the region [newsz, oldsz)
  if (newsz < oldsz) {
    uvmunmap(kpgtbl, newsz, (oldsz - newsz) / PGSIZE, 0);
    return 0;
  }

  // If growing, map the region [oldsz, newsz)
  for (va = oldsz; va < newsz; va += PGSIZE) {
    if ((upte = walk(upgtbl, va, 0)) == 0) panic("copyuvm2kvm: pte should exist");
    if ((*upte & PTE_V) == 0) panic("copyuvm2kvm: page not present");
